...
class Downloader {
  ...
  download(options, next, progress) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development' || process.env.DEBUG_PROD === 'true') {
        cmd = './extra_res/wget/wget.exe';
      }else {
        cmd = './wget/wget.exe';
      }
      console.log(`start wget with command line  ${cmd}`);
      ...
      console.log(`start wget with command options  ${JSON.stringify(commandOptions)}`);
      this.command = spawn(cmd, commandOptions);
      this.registerListeners(this.command, options, next, progress);
    } catch(e) {
      console.log(`start wget exception ${e}`);
      next(e, options);
    }
  }
  ...
  buildCommandOptions(options) {
    ...
    let cmdOptions = ['--no-check-certificate'];
  ...
  registerListeners(command, options, next, progress) {
    ...
    command.stdout.on('error', (err) => {
      console.log("process stdout error");
      commandError = err;
      command.kill("SIGINT");
    });
